...
// ============================================================
// AD 2 - LIFE JOURNEY EXPORT (NO "LIFE MOVES FAST" VERSION)
// ============================================================
...
const CONFIG = {
  ...
  overlayHtml: "html/ad-two-life-journey-overlay-no-life-text.html",

  // Output
  outputVideo: "edited-videos/ad-two-life-journey-no-life-text.mp4",
  ...
  // Timing (no "Life moves fast" - goes directly from clips to captions)
  clipsDuration: 6.96,      // Total duration of trimmed clips
  captionsStart: 7.0,       // Captions start right after clips
  audioResumeAt: 7.32,      // Audio timestamp for "and booking..."
  totalDuration: 16.0,      // Total ad duration (3s shorter without life text)
  ...
  // Temp files
  tempDir: "edited-videos/_temp_ad2_no_life",
  framesDir: "edited-videos/_frames_ad2_no_life",
...
async function exportAd() {
  ...
  console.log("");
  console.log("╔══════════════════════════════════════════════════╗");
  console.log("║  Ad 2 - Life Journey (No Life Text) - Export     ║");
  console.log("╚══════════════════════════════════════════════════╝");
  console.log("");
  ...
  // ============================================================
  // STEP 2: Create blue background video for caption/logo phases
  // ============================================================
  ...
  // ============================================================
  // STEP 4: Create edited audio track (NO SILENCE GAP)
  // ============================================================
  console.log("🎵 Step 4: Creating audio track...");

  // Audio structure (no silence):
  // 0 - 7.5s: Original audio (0 - 7.5s) - includes clip audio ending at "grandkids"
  // 7.5s - end: Audio from 7.32s onwards (no gap)

  const finalAudio = path.join(tempDir, "final_audio.m4a");

  // Concatenate first 7.5s + from 7.32s onwards (continuous, no silence)
  execSync(
...
async function createOverlayHtml(outputPath) {
  // Overlay HTML without "Life moves fast" - goes directly from clips to captions
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Phase 2 captions (directly after clips) */
    .caption-layer {
      position: absolute;
      inset: 0;
      display: flex;
...
<body>
  ...
  <script>
    ...
    // Timing: No "Life moves fast" - go directly from clips to captions
    const CLIPS_END = 6.96;
    const CAPTIONS_START = 7.0;
    ...
    // Captions now start at 7.0s (no 3s gap for "Life moves fast")
    // Audio at 7.32 plays at ad time 7.5s (after 0.5s transition)
    const captions = [
    ...
    window.setTime = function(t) {
      ...
      // Phase 2: Captions (7s to ~14s) - NO "Life moves fast"
      else if (t < CAPTIONS_START + (AUDIO_END - AUDIO_RESUME)) {
        ...
        // Map ad time to audio time: ad 7.0s -> audio 7.32s
        const audioTime = AUDIO_RESUME + (t - CAPTIONS_START);
